...
namespace WebAppComponents.Services
{
    public class CatalogService(HttpClient httpClient) : ICatalogService
    {
        ...
        {
            var uri = $"{remoteServiceBaseUrl}items/by?id={string.Join("&ids=", ids)}";
        ...
        {
            var uri = $"{remoteServiceBaseUrl}items/withsemanticrelevance?text={HttpUtility.UrlEncode(text)}&pageIndex={page}&pageSize={take}";
            var result = httpClient.GetFromJsonAsync<CatalogResult>(uri);
            return result;
        }
