...
namespace WebHooks.API.Services
{
    public class GrantUrlTesterService(IHttpClientFactory factory, ILogger<IGrantUrlTesterService> logger) : IGrantUrlTesterService
    {
        public async Task<bool> TestGrantUrl(string urlHook, string url, string token)
        {
            ...
            msg.Headers.Add("X-ecommerce-whtoken", token);
            ...
            try
            {
                ...
                var tokenReceived = response.Headers.TryGetValues("X-ecommerce-whtoken", out var tokenValues) ? tokenValues.FirstOrDefault() : null;
                var tokenExpected = string.IsNullOrEmpty(token) ? null : token;
