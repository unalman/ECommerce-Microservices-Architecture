...
namespace Catalog.FunctionalTests
{
    public sealed class CatalogApiTests : IClassFixture<CatalogApiFixture>
    {
        ...
        public async Task GetCatalogItemWithPartialName(double version)
        {
            ...
            Assert.Equal(0, result.PageIndex);
            Assert.Equal(5, result.PageSize);
            Assert.Contains("Alphine", result.Data.ToList().FirstOrDefault().Name);
        }
        ...
        public async Task AddCatalogItem(double version)
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("/apo/catalog/items", bodyContent, TestContext.Current.CancellationToken);
            response.EnsureSuccessStatusCode();
        ...
        public async Task DeleteCatalogItem(double version)
        {
            ...
            var responseStatus = response1.EnsureSuccessStatusCode();
